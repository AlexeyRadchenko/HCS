...
def styling_row (act_list, row_num, style):
    act_list[f'A{row_num}'].style = style
    act_list[f'B{row_num}'].style = style
    act_list[f'C{row_num}'].style = style
    act_list[f'D{row_num}'].style = style
...
async def grouping_and_sum_works(works, db_session):
    ...
    for work in works:
        print("###################################################", work)
        if work['workType'] == 'mainwork':
            if not group.get('main_work'):
                group['main_work'] = work['work'][2:]
                group['main_work_id'] = work['id']
                group['total_sum'] = float(work['sum']),
                group['works'] = [{ 'work_id': work['id'],
                                    'name_work': work['work'],
                                    'period': work['period'], 
            ...
            elif group.get('main_work_id') != work['id']:
                group_works.append(group)
                group = {'record_ids': []}
                main_work = await get_mainwork_by_id(db_session, work['mainwork_id'])
                group['main_work'] = main_work.work[2:]
                group['main_work_id'] = main_work.id
                group['total_sum'] = float(work['sum']),
                group['works'] = [{ 'work_id': work['id'],
                                    'name_work': work['work'],
                                    'period': work['period'], 
                ...
                if work['id'] in group['record_ids']:
                    ...
                    group['works'][double_index]['quantity'] = group['works'][double_index]['quantity'] + work['quantity']
                    group['works'][double_index]['sum'] = group['works'][double_index]['sum'] + work['sum']
                    continue    
                group['total_sum'] = group['total_sum'] + float(work['sum']),
                group['works'].append({
                    'work_id': work['id'],
                    'name_work': work['work'],
                    'period': work['period'], 
            ...
            if not group.get('main_work'):
                ...
                group['record_ids'].append(work['id'])
            elif group.get('main_work_id') != work['mainwork_id']:
                print("GO TO ANOTHER")
                main_work = await get_mainwork_by_id(db_session, work['mainwork_id'])
                group = {
                    'main_work': main_work.work[2:],
                    'main_work_id': main_work.id,
                    'total_sum': float(work['sum']),
                    'works': [{
                        'work_id': work['id'],
                        'name_work': work['work'],
                        'period': work['period'],
...
async def write_table_data(doc_sheet, write_data, start_write_row_num, db_session, year):
    ...
    cell_style.font = font
     ...
     # Создаем именованный стиль
    cell_style_left = NamedStyle(name="styled_cell_left")
    cell_style_left.border = border
    cell_style_left.alignment = alignment_left
    cell_style_left.font = font_bold 
    ...
    for group in group_works:
        print('!!!!!!!!!!!!!!!!!!!!!!!!!', len(group['works']))
        styling_row(doc_sheet, write_row, cell_style)
        ...
        for work in group['works']:
            ...
            doc_sheet[f'A{write_row}'].style = cell_style
            doc_sheet[f'B{write_row}'].value = work['period']
            doc_sheet[f'B{write_row}'].style = cell_style
            doc_sheet[f'C{write_row}'].value = work['quantity']
            doc_sheet[f'C{write_row}'].style = cell_style
            doc_sheet[f'D{write_row}'].value = work['unitcost']
            doc_sheet[f'D{write_row}'].style = cell_style
            doc_sheet[f'E{write_row}'].value = work['sum']
            doc_sheet[f'E{write_row}'].style = cell_style
            write_row += 1
        doc_sheet[f'A{write_row}'].value = 'Итого:'
        styling_row(doc_sheet, write_row, cell_style_left)
        doc_sheet.merge_cells(f'A{write_row}:D{write_row}')
        doc_sheet[f'E{write_row}'].style = cell_style
        total_sum = float(group['total_sum'][0]) if isinstance(group['total_sum'], tuple) else group['total_sum']
        doc_sheet[f'E{write_row}'].value = "{0:.2f}".format(total_sum)
        total_sum_of_groups += total_sum
        write_row += 1
    doc_sheet[f'A{write_row}'].value = f'Итого расходы за {year}:'
    styling_row(doc_sheet, write_row, cell_style_left)
    doc_sheet.merge_cells(f'A{write_row}:D{write_row}')
    doc_sheet[f'E{write_row}'].value = "{0:.2f}".format(total_sum_of_groups)
    doc_sheet[f'E{write_row}'].style = cell_style_left
    write_row += 1  
    return write_row
...
async def genereate_year_act_xlsx_file_v2(year, house, data, task_uuid, db_session):
    ...
    template_data = {
        ...
        'house_director': data[0]['director'] if data[0]['director'] else 'cовет МКД не создан ',
        'house_director_appartment': data[0]['director_appartment'] if data[0]['director_appartment'] else '--',
        'company_director': company_director_who,
        'company_name': company_name,
        'collective_direct_or_deliver_num': '---------',
    }
    #print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", template_data)
    footer_text = [
